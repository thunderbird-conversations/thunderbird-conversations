...
function MessageFromGloda(aConversation, aGlodaMsg, aLateAttachments) {
  ...
  this.isReplyAllEnabled =
    ...
    .filter(function (x) {
      let r = !(getIdentities.some(function (ident) ident.identity.email == x.value.toLowerCase())) && !(x.value in seen);
      seen[x.value] = null;
      return r;
    }).length > 1;
    ...
    MsgHdrToMimeMessage(aMsgHdr, null, function(aMsgHdr, aMimeMsg) {
      try {
        ...
        self.isReplyAllEnabled = 
          ...
          .filter(function (x) {
            let r = !(getIdentities.some(function (ident) ident.identity.email == x.email)) && !(x.email in seen);
            seen[x.email] = null;
            return r;
          })
          .length > 1;
