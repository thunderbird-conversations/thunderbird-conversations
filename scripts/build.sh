...
mkdir -p "../$DIST"
find .  -maxdepth 1 -regex $REGEXTENSIONS -exec cp {} ../$DIST/ \;
...
for a in "${DIRECTORIES[@]}"; do
  mkdir -p ../$DIST/${a}/
  find $a -regex $REGEXTENSIONS -exec cp {} ../$DIST/$a/ \;
done
for dir in _locales/*; do
  mkdir -p ../$DIST/${dir}/
  find $dir -regex $REGEXTENSIONS -exec cp {} ../$DIST/$dir/ \;
done
# This directory just needs the css file.
mkdir -p ../$DIST/content/components/compose
find . -name "*.css" -exec cp {} ../$DIST/{} \;
